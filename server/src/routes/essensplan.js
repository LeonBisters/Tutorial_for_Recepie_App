...
router.post("/", verifyToken, async (req, res) => {
  const allEssenPlans = await EssenPlanModel.find({});
  const newEssenPlan = new EssenPlanModel(req.body);

  const isWeekAlreadySet = allEssenPlans.some(
    (essensPlan) => essensPlan.wochenNummer === newEssenPlan.wochenNummer
  );

  console.log("isWeekAlreadySet", isWeekAlreadySet);

  try {
    if (!isWeekAlreadySet) {
      const response = await newEssenPlan.save();
      res.json(response);
    } else {
      res
        .status(400)
        .json({ message: "Wochenplan wurde für diese Woche bereits gesetzt." });
    }
  } catch (err) {
    console.log("ERROR POST ESSENSPLAN", err);

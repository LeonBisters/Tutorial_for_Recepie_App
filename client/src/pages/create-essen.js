...
export const CreateEssen = () => {
    ...
    const handleIngredientChange = (event, idx) => {
        ...
        const ingredients = essen.ingredients;
        ingredients[idx] = value;
        setEssen({...essen, ingredients});
    };

    const addIngredient = () => {
        setEssen({...essen, ingredients: [...essen.ingredients, ""]});
    };

    const onSubmit = async (event) => {
        event.preventDefault();
        try {
            await axios.post("http://localhost:3001/recipes", essen, {headers: {authorization: cookies.access_token}});
